...
1. **Discover the codebase:**
   - Use `list_dir` to find all files recursively
   - Use `view_file` to read every source file
...
3. **Fix everything automatically:**
   - Use `replace_file_content` or `multi_replace_file_content` to fix issues in files
   - Use `write_to_file` to add missing files (tests, configs, docs)
